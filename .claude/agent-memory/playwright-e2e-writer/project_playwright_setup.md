...
## Ticket list UI tests (added 2026-07-18)
...
- **Empty-state pattern**: the `helpdesk_test` DB is shared across the whole run and `fullyParallel: true` means other spec files may have already seeded tickets by the time an "empty list" test runs — genuinely emptying the table via cleanup isn't reliable. Instead, intercept the request at the network layer: `page.route('**/api/tickets', route => { if (route.request().method() !== 'GET') return route.continue(); return route.fulfill({ json: [] }); })` before `page.goto('/tickets')`. This still exercises the real frontend empty-state branch (`tickets.length === 0` in `TicketTable.tsx`, rendering `"No tickets found."`) without depending on real DB state. Use this same pattern for any other list page's empty-state test in this project.
- Test file: `e2e/tests/tickets/ticket-list.spec.ts`.
